...
- 이전에 같은 내용으로 `p2/job-weights-v0.1.1` 브랜치를 만들어 커밋·push까지 했다고 확인했으나, 이후 여러 차례 브랜치 전환·stash 처리 과정에서 실제로는 원격에 반영되지 않은 채 유실된 것을 확인해 값을 다시 적용. 이번 브랜치(v0.1.2)도 한 번 더 stash 충돌로 `occupations.json`이 예전 버전(0.1.0/24개)으로 되돌아갔던 것을 main 기준(`origin/main`, 0.2.0/40개)으로 복원.

### 검증

- 8개 직군 `axis_weight` 합 전부 1.0 확인, `pytest -q` 6/6 통과(J03 최상위 회귀 테스트 영향 없음).
- `occupations.json`이 main과 동일한 0.2.0(40개)인지 재확인.
