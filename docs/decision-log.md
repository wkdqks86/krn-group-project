...
- MBTI / 에니어그램 / Big Five / 현실 조건은 추천 점수에 넣지 않음
...
## 2026-08-22 · 제품 정의 유지, P3 뼈대만 이식

### 결정

- 채점 공식·문항·8직군·TOP 5는 그대로 둔다. `p3/`의 Big Five·10직군 엔진은 쓰지 않는다.
- 화면은 `engine.py`만 호출한다. 세션 시작, 선택 입력 저장, 채점, 직군 상세, 피드백이 여기서 버전과 함께 SQLite에 남는다.
- 결과 상단에 `heard_summary`(방금 답에서 읽은 것)를 둔다. 점수를 다시 계산하지 않는다.

### 버전

- 적합도 `engine_version`은 0.1.0 유지 (공식 불변)
- 진입점·저장 계약 추가

## 2026-08-22 · P3 백엔드 엔진 v0.2 병합

### 결정

- P3 산출물은 현재 발표용 `app.py`(12축·28문항·TOP5)와 채점 계약이 다르므로, 화면 엔진을 덮어쓰지 않고 `p3/` 패키지로 넣는다.
- P3가 기대하는 P2 v2 콘텐츠(`questions.json`·`job_profiles.json`·`result_copy.json`·페르소나 기준선)를 `p3/data`, `p3/fixtures`에 함께 둔다.
- 발표 앱의 `services/work24.py`에는 P3의 실패 전제(timeout 3초, 60초 캐시, 예외 시 스냅샷)만 반영한다. P5 직업 스냅샷은 유지한다.

### 버전

- P3 `ENGINE_VERSION`: 0.2
